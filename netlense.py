...
class PriViNetLens:
    def __init__(self, iface=None, bpf_filter=None, no_geo=False):
        ...
        # ── Unsecure port tracker ─────────────────────────────────────────────
        # Track (src_ip, port) pairs already flagged — prevents per-packet inflation
        self._unsecure_alerted = set()
    ...
    def packet_handler(self, pkt):
        ...
        with self.state_lock:
            ...
            # Only increment threat_count once per unique (src_ip, port) pair
            # Prevents per-packet inflation from ongoing sessions (e.g. HTTP stream)
            if port in UNSECURE_PORTS:
                unsecure_key = (ip_layer.src, port)
                if unsecure_key not in self._unsecure_alerted:
                    self._unsecure_alerted.add(unsecure_key)
                    self.threat_count += 1
                    self.threat_feed.append((
                        time.strftime("%H:%M:%S"),
                        f"[bold red]!! UNSECURE PROTOCOL: {ip_layer.src} → "
                        f"{UNSECURE_PORTS[port]} (port {port}) !![/bold red]"
                    ))
...
def authorization_gate():
    ...
    console.print(Panel(gate_text, border_style="red",
                        title="[bold red]PriVi-NetLens | PriViSecurity[/bold red]"))
